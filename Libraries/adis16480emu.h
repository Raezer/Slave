#ifndef ADIS16480EMU_H
#define ADIS16480EMU_H
...
public:
	adis16480_spi(SPI& _spi, PinName _cs);
	bool init();
	void select();
	void deselect();
	bool need_restart();
	bool restart();
	unsigned int whoami();
	void read_all_mpu_data(MpuDataArray* a_res);
...
private:
	...
	static float _OmegaCode2Float(int32_t a);
	static float _AccelCode2Float(int32_t a);
	static float _MagnetometerCode2Float(int16_t a);
	static float _PressureCode2Float(int32_t a);
	static float _TemperatureCode2Float(int16_t a);
