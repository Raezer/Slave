...
__STATIC_INLINE int ssp_disable(spi_t *obj);
__STATIC_INLINE int ssp_enable(spi_t *obj);
...
void spi_format(spi_t *obj, int bits, int mode, int slave) {
	int polarity = 0, phase = 0;
	
    MBED_ASSERT(((bits == 8) || (bits == 16)) && ((mode >= 0) && (mode <= 3)));
    ssp_disable(obj);

    polarity = (mode & 0x2) ? 1 : 0;
    phase = (mode & 0x1) ? 1 : 0;
...
static __inline uint32_t __builtin_ffs(uint32_t a)
	{
	uint32_t mask = 0x1, res = 1;		
...
void spi_frequency(spi_t *obj, int hz) {
	...
	uint32_t PCLK = SystemCoreClock, divisor = 1, lsbIndex = 0, baud_rate = 0;
    ...
    switch ((int)obj->spi) {
        case SPI_1: PCLK = SystemCoreClock >> 1; break;
        case SPI_2: PCLK = SystemCoreClock >> 2; break;
        case SPI_3: PCLK = SystemCoreClock >> 2; break;
    }
...
__STATIC_INLINE int ssp_readable(spi_t *obj) {
    return obj->spi->SR & SPI_SR_RXNE;
}

__STATIC_INLINE int ssp_writeable(spi_t *obj) {
    return obj->spi->SR & SPI_SR_TXE;
}

__STATIC_INLINE void ssp_write(spi_t *obj, int value) {
    while (!ssp_writeable(obj));
    obj->spi->DR = value;
}

__STATIC_INLINE int ssp_read(spi_t *obj) {
    while (!ssp_readable(obj));
    return obj->spi->DR;
}

__STATIC_INLINE int ssp_busy(spi_t *obj) {
